...
async function main() {
    ...
    const [fullStackGoal, dataGoal] = await Promise.all([
        prisma.careerGoal.create({
            data: {
                ...
                description: 'Build complete web applications from frontend UI to backend APIs and databases. Master modern frameworks like React and Node.js to create scalable, production-ready systems.',
            },
        }),
        prisma.careerGoal.create({
            data: {
                name: 'Data Scientist',
                description: 'Analyze complex datasets, build machine learning models, and extract actionable insights. Develop expertise in Python, statistics, and ML frameworks to solve real-world data problems.',
            },
        }),
    ]);
    ...
    const saltRounds = 12;
    const hashedPassword = await bcryptjs_1.default.hash('Password123!', saltRounds);
    ...
    const edges = await Promise.all(edgesTo20.map(([parent, child]) => prisma.skillEdge.create({
        data: {
